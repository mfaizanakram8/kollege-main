...
const Paper = () => {
    ...
    const getPapers = async () => {
      try {
        ...
        if (user.role === "student") {
          response = await axios.get(`paper/student/${user._id}`);
        } else if (user.role === "HOD" || user.role === "teacher") {
          response = await axios.get(`paper/department/${user.department}`);
        }
  ...
  return (
    <main className="paper">
      ...
      {error && <p className="text-red-500">Error loading papers: {error.message}</p>}
    </main>
  );
};
